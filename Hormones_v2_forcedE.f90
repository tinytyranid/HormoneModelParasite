!*******************************************************************************
!This is an version of the hormone main code that is used for experiments
!
! Things that are changed in this version of the code:
! - Individuals start in the average environment: (E_categories+1) / 2
! - Individuals are forced through environments during their whole life
! -- Individual 1 is forced through a poor environment
! -- Individual 2 is forced through a average environment
! -- Individual 3 is forced through a good environment 
! -- Individual 4 is experiencing the normal environment they where optimised for
!
!*******************************************************************************
...
program Hormones_v2
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !VALUES USED FOR EXPERIMENTATION
  integer, parameter       :: EX_n = 4 !Number of individuals uses for experiments
  real(kind=RP), parameter :: EX_E_min_ind(EX_n) = (/  3.538462_RP, 5.653846_RP, 7.769231_RP,  1._RP/)
  real(kind=RP), parameter :: EX_E_max_ind(EX_n) = (/  7.769231_RP, 9.884615_RP,     12.0_RP,  12._RP/)
  real(kind=RP), parameter :: EX_E_scale = 1.0_RP !Scaling for the environments
  real(kind=RP), parameter :: EX_length = 9.5_RP !Individual length for experiment start
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  elseif (forwardOnly == "y") then
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    E_mean = 1._RP+real((E_categories-1),RP)/2._RP
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !Overwriting all environments so that all individuals experience the
  ! same environment as individual 1
  EXPR_1_LOOP: do n=1, n_max, 1 
    environment(n,:) = environment(1,:)
  end do EXPR_1_LOOP
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  Individ_f_LOOP: do n = 1, n_max, 1
    ...
    Time_f_LOOP: do t = 1, t_max, 1
      ...
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !Until EX_length everyone is as the average individual
      if (n <= EX_n .AND. length <= EX_length) then
        E_real = ( (E_real - 1._RP) / (E_categories - 1._RP) ) * (EX_E_max_ind(2) - EX_E_min_ind(2)) + EX_E_min_ind(2)
      end if
      
      !From EX_length
      if (n <= EX_n .AND. length >= EX_length) then
        E_real = ( (E_real - 1._RP) / (E_categories - 1._RP) ) * (EX_E_max_ind(n) - EX_E_min_ind(n)) + EX_E_min_ind(n)
      end if
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      intL = max(1,min(int(L_cat),L_categories-2))          !... integer part   !### subtracted 1 from L_categories to use strategy from L_max-1 so they reach max length, commented else-statement below
